...
function destroy() {
    form.transform(() => ({ _method: 'DELETE' }))
        .post(`/admins/managers/${targetId.value}`, {
            onSuccess: () => {
...
<div>
    ...
    <div class="mb-6 flex items-end justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Manage managers</h1>
            <p class="text-sm text-slate-500 mt-1">
                {{ props.managers.total }} managers total
            </p>
        </div>

        <Link href="/admins/managers/create" :class="{ 'pointer-events-none opacity-50': form.processing }">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-slate-50 border-b">
                    ...
                    <th @click="changeSort('email')" class="cursor-pointer px-5 py-3">
                        Email
                    </th>

                    <th @click="changeSort('national_id')" class="cursor-pointer px-5 py-3">
                        National ID
                    </th>

                    <th class="cursor-pointer px-5 py-3">
                        Image
                    </th>
            ...
            <tbody>
                ...
                >
                    ...
                    <td class="px-5 py-4">
                      <img
                        :src="manager.avatar_image ? `/storage/${manager.avatar_image}` : '/images/default.png'"
                    ...
                    <td class="px-5 py-4">
                        <div class="flex gap-2">
                            ...
                            <Link :href="`/admins/managers/${manager.id}`">
    ...
    <div class="flex justify-between mt-4">
        <Button
            ...
            @click="fetchData({ page: props.managers.current_page - 1 })"
        >
            Prev
        </Button>

        <span>
            Page {{ props.managers.current_page }} of {{ props.managers.last_page }}
        </span>
