...
const form = useForm({
    name: '',
    manager_id: user?.roles?.includes('admin') ? null : user?.value?.id,
  })
    ...
    <div class="w-full">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Fill in the details to create a new floor.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5 w-full">
            ...
            <div class="space-y-2" v-if="user?.roles?.includes('admin')">
                <Label>Manager</Label>
